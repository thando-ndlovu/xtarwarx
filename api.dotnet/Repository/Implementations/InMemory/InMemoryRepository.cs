...
namespace Repository.Implementations.InMemory
{
	...
	{
		public InMemoryRepository(IServiceProvider servicesrovider)
		{
			ServiceProvider = servicesrovider;
		}

		protected IServiceProvider ServiceProvider { get; }
		protected InMemoryRepositoryDbContext DB => ServiceProvider.GetRequiredService<InMemoryRepositoryDbContext>();
		...
		public static void Configure(IServiceCollection services, InMemoryRepositoryOptions options)
		{
			services.AddDbContext<InMemoryRepositoryDbContext>(
				...
				optionsAction: dbcontextoptionsbuilder =>
				{
					...
					dbcontextoptionsbuilder.UseInMemoryDatabase(
						inMemoryOptionsAction: options.DbContextOptions,
						databaseName: options.DatabaseName ?? nameof(InMemoryRepository));
				});
